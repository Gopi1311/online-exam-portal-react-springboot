...
const Homecontentteacher = ({ Toggle }) => {
  ...
  useEffect(() => {
    const Testcard = async () => {
      await axios.get('http://localhost:8081/test/view')
        .then((res) => {
          setTests(res.data);
  ...
  const handleDelete = async (testId) => {
    await axios.delete(`http://localhost:8081/test/removeTest/${testId}`)
      .then((res) => {
            ...
            <div className="row g-3 my-2">
              {tests.map((test, index) => (
                <div className="col-md-3 p-2" key={index}>
                  <div className="card shadow-sm d-flex flex-column justify-content-between align-items-center rounded mx-3" style={{ height: '300px' }}>
                    <div className="card-body d-flex flex-column justify-content-between w-100">
                      <div>
                        <Link to={`/viewtestteacher/${test.id}`} className="text-decoration-none text-dark">
                          <h3 className="fs-2 testcard" style={{ wordWrap: 'break-word' }}>
                      ...
                      <div className="mt-auto">
                        <button className="btn btn-danger w-100" onClick={() => handleDelete(test.id)}>
                          Delete
                        </button>
                      </div>
                    </div>
